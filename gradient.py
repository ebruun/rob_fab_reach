...
def plot_surf(R_pnts, R_scores, pnts_save, L_max, num_pnts):
    ...
    for p,lam in zip(R_pnts,R_scores):
        l += lam * relu(1 - jnp.sqrt((X-p[0])**2 + (Y-p[1])**2)/L_max)
...
def func_reach_score_plus(pnts,R_pnts,R_scores,L_max, L_orig=False):
    ...
    l_2 = -jnp.abs((L_orig - jnp.sqrt((pnts[0,0]-pnts[1,0])**2 + (pnts[0,1]-pnts[1,1])**2))/L_orig)

    return 1.0*sum(l_1) + 0.2*l_2
...
def backtrack(R_pnts, R_scores, pnts, L_max, grad, flag = True):
    ...
    print("--btrack: alpha = (0.5)^0, OLD: {:.3f} - NEW: {:.3f}".format(sum(reach_scores), sum(reach_scores_new)))

    alphas = np.ones(pnts.shape[0])
    cnts = np.zeros(pnts.shape[0])

    if flag:
        while np.any(reach_scores_new<reach_scores) or np.any(reach_scores_new == 0): # if 0, then it can go outside function range
            
            cnts = np.array([cnt+bool for bool,cnt in zip(reach_scores_new<reach_scores,cnts)])
            
            alphas = (1/2)**cnts

            reach_scores_new = func_reach_score2(pnts+(grad.T * alphas).T,R_pnts,R_scores, L_max)
            
            print("--btrack: alpha = (0.5)^{}, OLD: {:.3f} - NEW: {:.3f}".format(cnts, sum(reach_scores), sum(reach_scores_new)))
...
def backtrack2(R_pnts, R_scores, pnts, L_max, grad, L_orig, flag = True):
    ...
    print("--btrack: alpha = (0.5)^0, OLD: {:.3f} - NEW: {:.3f}".format(reach_scores, reach_scores_new))

    alphas = np.ones(pnts.shape[0])
    cnts = np.zeros(pnts.shape[0])

    if flag:
        while reach_scores_new <  reach_scores or reach_scores_new == 0: # if 0, then it can go outside function range
            
            cnts += 1
            
            alphas = (1/2)**cnts

            reach_scores_new = func_reach_score_plus(pnts+(grad.T * alphas).T,R_pnts,R_scores, L_max, L_orig)
            
            print("--btrack: alpha = (0.5)^{}, OLD: {:.3f} - NEW: {:.3f}".format(cnts, reach_scores, reach_scores_new))
...
def main(R_pnts, R_scores, pnts, L_max):
    ...
    pnts_save = np.hstack((pnts,func_reach_score2(pnts,R_pnts,R_scores,L_max).reshape(num_pnts,1)))
    ...
    for i in range(0,50):
        ...
        #1. EVAL FUNC
        reach_score, grad = value_and_grad(func_reach_score_plus,(0))(pnts,R_pnts,R_scores,L_max, L_orig)
